 ...
 * this class.
 * <p>
 * based on http://developer.amazonwebservices.com/connect/entry.jspa?externalID
 * =1468&categoryID=114
 * </p>
 * 
...
public class AWSSecurityHandler implements SOAPHandler<SOAPMessageContext> {
	 ...
	 * 
	 */
	public void setKeys(String inAWSKey, String inAWSSecrectKey) {
	 ...
	 * calculating message authentication signatures.
	 */
	public AWSSecurityHandler(String inAWSKey, String inAWSSecrectKey) {
	 ...
	 * Log Message.
	 */
	private void logMessage(String text, SOAPMessage message) {
	...
	public boolean handleMessage(SOAPMessageContext context) {
		...
		try {
			...
			// ADD ACCESS KEY -------------------------------------------------
			Name aKeyName = aEnvelope.createName("AWSAccessKeyId", NAMESPACE_AWS_PREFIX,
					NAMESPACE_AWS);
			SOAPHeaderElement aKey = aHeader.addHeaderElement(aKeyName);
			aKey.addTextNode(s_key);

			// ADD TIMESTAMP --------------------------------------------------
			Name aTimestampName = aEnvelope.createName("Timestamp", NAMESPACE_AWS_PREFIX,
					NAMESPACE_AWS);
			SOAPHeaderElement aTimestamp = aHeader.addHeaderElement(aTimestampName);
			aTimestamp.addTextNode(aTimestampStr);

			// ADD SIGNATURE --------------------------------------------------
			Name aSignatureName = aEnvelope.createName("Signature", NAMESPACE_AWS_PREFIX,
					NAMESPACE_AWS);
	 ...
	 */
	public String calculateSignature(String inSOAPAction, String inTimeStamp) throws Exception {
		// FROM
