...
public class JmxToAllBeansPostprocessor implements BeanPostProcessor, InitializingBean {
	...
	private static final Logger log = Logger.getLogger(JmxToAllBeansPostprocessor.class);
	...
	public Object postProcessAfterInitialization(Object bean, String beanName)
		...
		if (mbean != null) {
			try {
				mbeanServer.registerMBean(mbean, naming.getObjectName(bean, beanName));
	...
	ModelMBean createMBean(Object bean, String beanName) {
		...
		try {
			if (AopUtils.getTargetClass(bean) == null) {
				// Would fail
				log.debug(String.format("Skipping jmx support for gateway bean: %s. ", beanName));
				return null;
			}
